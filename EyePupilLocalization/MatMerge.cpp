...
cv::Mat MatMerge(cv::Mat& img_left, cv::Mat& img_right)
{
	...
	img_merge.create(size, img_right.type());
	...
	//else
	{
		...
		img_right.colRange(0, img_right.cols).copyTo(outImg_right);
	}
	
	//img_left.colRange(0, img_left.cols).copyTo(img_merge.colRange(0, img_left.cols));
	//img_right.colRange(0, img_right.cols).copyTo(img_merge.colRange(img_left.cols + 1, img_merge.cols));
	return img_merge;
}